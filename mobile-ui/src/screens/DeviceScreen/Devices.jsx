...
function DevicesScreen({ navigation }) {
    ...
    const renderRoom = ({ item, index }) => (
        <Button
            ...
            onPress={() => {
                setCurrentRoom(item.key);
            }}
        >
            <Text
                className={
                    item.key === currentRoom
    ...
    const renderItem = ({ item, index }) => (
        <View
            className="flex-1 flex justify-between items-center m-[8px]"
            key={"devicecard" + index}
        >
            <DeviceCard
                device={item}
                theme={item.favorite ? "light" : "dark"}
                onPress={() => {
                    navigation.push(item.deviceName + item.key);
                }}
            />
        </View>
    );
